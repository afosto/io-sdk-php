<?php

declare(strict_types=1);

/*
 * This file has been auto generated by Jane,
 *
 * Do no edit it directly.
 */

namespace Afosto\Sdk\Normalizer;

use Symfony\Component\Serializer\Normalizer\DenormalizerAwareInterface;
use Symfony\Component\Serializer\Normalizer\DenormalizerAwareTrait;
use Symfony\Component\Serializer\Normalizer\DenormalizerInterface;
use Symfony\Component\Serializer\Normalizer\NormalizerAwareInterface;
use Symfony\Component\Serializer\Normalizer\NormalizerAwareTrait;
use Symfony\Component\Serializer\Normalizer\NormalizerInterface;

class LcsShipmentDetailsNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    use DenormalizerAwareTrait;
    use NormalizerAwareTrait;

    public function supportsDenormalization($data, $type, $format = null)
    {
        return $type === 'Afosto\\Sdk\\Model\\LcsShipmentDetails';
    }

    public function supportsNormalization($data, $format = null)
    {
        return get_class($data) === 'Afosto\\Sdk\\Model\\LcsShipmentDetails';
    }

    public function denormalize($data, $class, $format = null, array $context = [])
    {
        if (!is_object($data)) {
            return null;
        }
        $object = new \Afosto\Sdk\Model\LcsShipmentDetails();
        if (property_exists($data, 'id') && $data->{'id'} !== null) {
            $object->setId($data->{'id'});
        }
        if (property_exists($data, 'is_concept') && $data->{'is_concept'} !== null) {
            $object->setIsConcept($data->{'is_concept'});
        }
        if (property_exists($data, 'number') && $data->{'number'} !== null) {
            $object->setNumber($data->{'number'});
        }
        if (property_exists($data, 'method') && $data->{'method'} !== null) {
            $object->setMethod($this->denormalizer->denormalize($data->{'method'}, 'Afosto\\Sdk\\Model\\LcsMethod', 'json', $context));
        }
        if (property_exists($data, 'contact_id') && $data->{'contact_id'} !== null) {
            $object->setContactId($data->{'contact_id'});
        }
        if (property_exists($data, 'addressing') && $data->{'addressing'} !== null) {
            $object->setAddressing($this->denormalizer->denormalize($data->{'addressing'}, 'Afosto\\Sdk\\Model\\LcsShipmentDetailsAddressing', 'json', $context));
        }
        if (property_exists($data, 'expected_at') && $data->{'expected_at'} !== null) {
            $object->setExpectedAt(\DateTime::createFromFormat("Y-m-d\TH:i:sP", $data->{'expected_at'}));
        }
        if (property_exists($data, 'stack_id') && $data->{'stack_id'} !== null) {
            $object->setStackId($data->{'stack_id'});
        }
        if (property_exists($data, 'is_collected') && $data->{'is_collected'} !== null) {
            $object->setIsCollected($data->{'is_collected'});
        }
        if (property_exists($data, 'is_in_transit') && $data->{'is_in_transit'} !== null) {
            $object->setIsInTransit($data->{'is_in_transit'});
        }
        if (property_exists($data, 'is_delivered') && $data->{'is_delivered'} !== null) {
            $object->setIsDelivered($data->{'is_delivered'});
        }
        if (property_exists($data, 'is_verified') && $data->{'is_verified'} !== null) {
            $object->setIsVerified($data->{'is_verified'});
        }
        if (property_exists($data, 'items') && $data->{'items'} !== null) {
            $values = [];
            foreach ($data->{'items'} as $value) {
                $values[] = $this->denormalizer->denormalize($value, 'Afosto\\Sdk\\Model\\LcsShipmentItem', 'json', $context);
            }
            $object->setItems($values);
        }
        if (property_exists($data, 'backorders') && $data->{'backorders'} !== null) {
            $values_1 = [];
            foreach ($data->{'backorders'} as $value_1) {
                $values_1[] = $this->denormalizer->denormalize($value_1, 'Afosto\\Sdk\\Model\\LcsBackorderItem', 'json', $context);
            }
            $object->setBackorders($values_1);
        }
        if (property_exists($data, 'parcels') && $data->{'parcels'} !== null) {
            $values_2 = [];
            foreach ($data->{'parcels'} as $value_2) {
                $values_2[] = $this->denormalizer->denormalize($value_2, 'Afosto\\Sdk\\Model\\LcsParcel', 'json', $context);
            }
            $object->setParcels($values_2);
        }
        if (property_exists($data, 'secret') && $data->{'secret'} !== null) {
            $object->setSecret($data->{'secret'});
        }
        if (property_exists($data, 'created_at') && $data->{'created_at'} !== null) {
            $object->setCreatedAt(\DateTime::createFromFormat("Y-m-d\TH:i:sP", $data->{'created_at'}));
        }
        if (property_exists($data, 'updated_at') && $data->{'updated_at'} !== null) {
            $object->setUpdatedAt(\DateTime::createFromFormat("Y-m-d\TH:i:sP", $data->{'updated_at'}));
        }

        return $object;
    }

    public function normalize($object, $format = null, array $context = [])
    {
        $data = new \stdClass();
        if (null !== $object->getId()) {
            $data->{'id'} = $object->getId();
        }
        if (null !== $object->getIsConcept()) {
            $data->{'is_concept'} = $object->getIsConcept();
        }
        if (null !== $object->getNumber()) {
            $data->{'number'} = $object->getNumber();
        }
        if (null !== $object->getMethod()) {
            $data->{'method'} = $this->normalizer->normalize($object->getMethod(), 'json', $context);
        }
        if (null !== $object->getContactId()) {
            $data->{'contact_id'} = $object->getContactId();
        }
        if (null !== $object->getAddressing()) {
            $data->{'addressing'} = $this->normalizer->normalize($object->getAddressing(), 'json', $context);
        }
        if (null !== $object->getExpectedAt()) {
            $data->{'expected_at'} = $object->getExpectedAt()->format("Y-m-d\TH:i:sP");
        }
        if (null !== $object->getStackId()) {
            $data->{'stack_id'} = $object->getStackId();
        }
        if (null !== $object->getIsCollected()) {
            $data->{'is_collected'} = $object->getIsCollected();
        }
        if (null !== $object->getIsInTransit()) {
            $data->{'is_in_transit'} = $object->getIsInTransit();
        }
        if (null !== $object->getIsDelivered()) {
            $data->{'is_delivered'} = $object->getIsDelivered();
        }
        if (null !== $object->getIsVerified()) {
            $data->{'is_verified'} = $object->getIsVerified();
        }
        if (null !== $object->getItems()) {
            $values = [];
            foreach ($object->getItems() as $value) {
                $values[] = $this->normalizer->normalize($value, 'json', $context);
            }
            $data->{'items'} = $values;
        }
        if (null !== $object->getBackorders()) {
            $values_1 = [];
            foreach ($object->getBackorders() as $value_1) {
                $values_1[] = $this->normalizer->normalize($value_1, 'json', $context);
            }
            $data->{'backorders'} = $values_1;
        }
        if (null !== $object->getParcels()) {
            $values_2 = [];
            foreach ($object->getParcels() as $value_2) {
                $values_2[] = $this->normalizer->normalize($value_2, 'json', $context);
            }
            $data->{'parcels'} = $values_2;
        }
        if (null !== $object->getSecret()) {
            $data->{'secret'} = $object->getSecret();
        }
        if (null !== $object->getCreatedAt()) {
            $data->{'created_at'} = $object->getCreatedAt()->format("Y-m-d\TH:i:sP");
        }
        if (null !== $object->getUpdatedAt()) {
            $data->{'updated_at'} = $object->getUpdatedAt()->format("Y-m-d\TH:i:sP");
        }

        return $data;
    }
}
